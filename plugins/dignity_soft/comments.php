...
$CI->db->order_by('dignity_soft_comments_datecreate', 'desc');
if (!is_login())
{
	$CI->db->where('dignity_soft_comments_approved', true);	
}
...
{	
	...
	{	
		...
		$catout .= '<li>';
		$catout .= '<p>';
		$catout .= '<a href="' . getinfo('siteurl') . $options['slug'] . '/view/' . $entry['dignity_soft_comments_thema_id'] . '">' . blogs_cleantext($entry['dignity_soft_comments_text']) . '</a>'
			. '<br>' . t(' от ', __FILE__) . '<a href="' . getinfo('siteurl') . 'users/' . $entry['dignity_soft_comments_comuser_id'] . '">' . $entry['comusers_nik'] . '</a>' . t(' в ', __FILE__) . mso_date_convert($format = 'H:i → d.m.Y', $entry['dignity_soft_comments_datecreate']);
		if (is_login())
		{	
			# удаление
			if ( $post = mso_check_post(array('f_session_id', 'f_submit_dignity_soft_comments_delete')) )
			{
				...
				if ( !isset($post['f_id'])) $post['f_id'] = $entry['dignity_soft_comments_id'];
			...
			# одобрить
			if ( $post = mso_check_post(array('f_session_id', 'f_submit_dignity_soft_comments_approved')) )
			{
				mso_checkreferer();
				...
				$CI->db->where('dignity_soft_comments_id', $post['f_comments_id']);
				$CI->db->update('dignity_soft_comments', $data);
			...
			$form .= '<input type="hidden" name="f_comments_id" value="' . $entry['dignity_soft_comments_id'] . '" />';
			
			if (!$entry['dignity_soft_comments_approved'])
			{
				$form .= '<input type="submit" name="f_submit_dignity_soft_comments_approved" onClick="if(confirm(\'' . t('Одобрить?', __FILE__) . ' ' . t('Комментарий №', __FILE__) . $entry['dignity_soft_comments_id'] . '\')) {return true;} else {return false;}" value="' . t('Одобрить', __FILE__) . '">';	
			}
			
			$form .= '<input type="submit" name="f_submit_dignity_soft_comments_delete" onClick="if(confirm(\'' . t('Удалить?', __FILE__) . ' ' . t('Комментарий №', __FILE__) . $entry['dignity_soft_comments_id'] . '\')) {return true;} else {return false;}" value="' . t('Удалить', __FILE__) . '">';
			$form .= '</form>';
