...
$options_key = 'plugin_dignity_soft';
...
if ($post = mso_check_post(array('f_session_id', 'f_submit_dignity_soft_category')) )
{
	// проверяем реферала
	mso_checkreferer();	
	...
	$ins_data = array (
			'dignity_soft_category_name' => $post['f_dignity_soft_category_name'],
			'dignity_soft_category_description' => $post['f_dignity_soft_category_description'],
			'dignity_soft_category_position' => $post['f_dignity_soft_category_position']
			);
	
	// добавляем в базу
	$res = ($CI->db->insert('dignity_soft_category', $ins_data)) ? '1' : '0';
...
$form .= '<p><strong>' . t('Название:', __FILE__) . '</strong><span style="color:red;">*</span><br>
	<input name="f_dignity_soft_category_name" type="text" style="width:50%" required="required"></p>';
	
$form .= '<p><strong>' . t('Описание:', __FILE__) . '</strong><br>
	<input name="f_dignity_soft_category_description" type="text" style="width:50%"></p>';
	
$form .= '<p><strong>' . t('Порядок:', __FILE__) . '</strong><br>
	<input name="f_dignity_soft_category_position" type="text" style="width:50%"></p>';

// конец формы
$form .= '<p><input type="submit" class="submit" name="f_submit_dignity_soft_category" value="' . t('Добавить', __FILE__) . '">';
$form .= '</form>';	
...
$CI->db->order_by('dignity_soft_category_position', 'desc');
$query = $CI->db->get();
...
{	
	...
	{
		$id = $rw['dignity_soft_category_id'];
		
		# удаление
		if ( $post = mso_check_post(array('f_session_id', 'f_submit_dignity_soft_category_delete')) )
		{
			mso_checkreferer();
			
			if ( !isset($post['f_id'])) $post['f_id'] = $id;
			
			$CI->db->where('dignity_soft_category_id', $post['f_id']);
			$CI->db->delete('dignity_soft_category');
		...
		# редактирование
		if ( $post = mso_check_post(array('f_session_id', 'f_submit_dignity_soft_category_edit')) )
		{
			mso_checkreferer();
			...
			$data = array (
					'dignity_soft_category_name' => htmlspecialchars($post['f_dignity_soft_category_name']),
					'dignity_soft_category_description' => htmlspecialchars($post['f_dignity_soft_category_description']),
					'dignity_soft_category_position' => htmlspecialchars($post['f_dignity_soft_category_position']),
			);
			
			$CI->db->where('dignity_soft_category_id', $post['f_id']);
			
			if ($CI->db->update('dignity_soft_category', $data ) )
		...
		$form = '';
		$form .= '<form action="" method="post">' . mso_form_session('f_session_id');
		$form .= '<p>' . t('Название:', __FILE__) . '<br> <input name="f_dignity_soft_category_name" type="text" style="width:90%" value="' . $rw['dignity_soft_category_name'] . '"></p>';
		$form .= '<p>' . t('Описание:', __FILE__) . '<br> <input name="f_dignity_soft_category_description" type="text" style="width:90%" value="' . $rw['dignity_soft_category_description'] . '"></p>';
		$form .= '<p>' . t('Позиция:', __FILE__) . '<br> <input name="f_dignity_soft_category_position" type="text" style="width:90%" value="' . $rw['dignity_soft_category_position'] . '"></p>';
		$form .= '<input type="hidden" name="f_id" value="' . $id . '" />';
		$form .= '<input type="submit" name="f_submit_dignity_soft_category_edit" value="' . t('Сохранить', __FILE__) . '" style="margin: 10px 0;">';
		$form .= ' <input type="submit" name="f_submit_dignity_soft_category_delete" onClick="if(confirm(\'' . t('Удалить?', __FILE__) . ' ' . t('Заявку №', __FILE__) . $rw['dignity_soft_category_id'] . '\')) {return true;} else {return false;}" value="' . t('Удалить', __FILE__) . '">';
		$form .= '</form>';
