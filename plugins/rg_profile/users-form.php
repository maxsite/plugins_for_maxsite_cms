...
if ($comuser_info) {
    ...
    if ($f = mso_page_foreach('users-form')) require($f); // подключаем кастомный вывод
        ...
        {
            echo '<form method="post" class="comusers-form fform" enctype="multipart/form-data">' . mso_form_session('f_session_id');
            ...
            if ($login_form) // нужно отобразить форму
            {
                ...
                if (mso_get_option('gravatar_only', 'templates', 0))
                    ...
                    if (!empty($comusers_avatar_url)) echo '<p><span class="ffirst ftitle">' . tf('Загрузить аватарку') . '</span><span id="ava_upload"><img src="' . $comusers_avatar_url . '" style="height: 80px;"> <a href="javascript:void(null);" onclick="javascript:newupload(\''.$comusers_avatar_url.'\');">Удалить и загрузить другую</a><input type="hidden" name="f_comusers_avatar_url" value="'.$comusers_avatar_url.'"></p>';
                    else echo '<p><span class="ffirst ftitle">' . tf('Загрузить аватарку') . '</span><span><input type="file" name="f_comusers_avatar_url"></p>';
                }
...
<script type="text/javascript">
    function newupload(url) {
        $("span#ava_upload").html('<input type="file" name="f_comusers_avatar_url">');
        $.post("/users/delfile", { url: url });
    }
</script>