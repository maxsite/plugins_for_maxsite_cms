		...
		$.ajax({
			...
			success:function(json){
				if( json.res != '' ){
					...
					current_pg++; $(el).attr('data-current', current_pg); // увеличиваем номер порции на единицу
					if( json.hide ) $(el).parents('.mso-comment-more').hide(); // если последняя страница, скрываем кнопку
					SetReplyButtons();
					SetRatingButtons();		
				} else {
					$(el).parents('.mso-comment-more').hide(); // если мы дошли до последней страницы записей, скроем кнопку
				}
			}
		});
	});
