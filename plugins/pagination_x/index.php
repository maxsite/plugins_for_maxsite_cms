...
function pagination_x_go($r = array()) 
{
	...
	{
		$out = str_replace(
				array('%TITLE_PREV%', '%PREV%', '%NEXT%', '%TITLE_NEXT%'),
				$r['format'],
				$out);
...
function _pagination($pre_text, $max, $page_number, $base_url, $diappazon = 4, $url_first = '', $page_u = '', $sep = ' &middot; ', $home_url = '', $sep2 = ' | ') 
{
	...
	else 
	{
		...
		$first_link = '<a class="pagination-prev" href="' . $prev . '"%TITLE_PREV%><span style="display:none;">%PREV%</span></a><span style="display:none;">' . $sep2 . '</span>';
	...
	{
		$last_link =  '<span style="display:none;">' . $sep2 . '</span><a class="pagination-next" href="' . $base_url . $page_u . $next_link_page . '"%TITLE_NEXT%><span style="display:none;">%NEXT%</span></a>';
...
function pagination_x_mso_options() 
{
	...
	mso_admin_plugin_options('plugin_pagination_x', 'plugins', 
		array(
			...
			'format_title' => array(
							'type' => 'checkbox', 
							'name' => 'title для «предыдущая / следующая»', 
							'description' => '', 
							'default' => '0'
						),
			'sep' => array(
