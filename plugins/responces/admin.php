...
<?php
	...
	$a  = mso_admin_link_segment_build($plugin_url, '', t('Настройки модуля отзывов'), 'select') . ' | ';
	$a .= mso_admin_link_segment_build($plugin_url, 'edit', t('Редактирование отзывов'), 'select');
	echo $a;
?>
</div>

<h1><?= t('Книга отзывов и предложений') ?></h1>
<p class="info"><?= t('Плагин позволяет организовать на вашем сайте книгу отзывов и предложений.') ?></p>

<?php
		$options = mso_get_option($options_key, 'plugins', array());

		if ( !isset($options['text']) ) $options['text'] = t("<h1>Отзывы и предложения</h1>\n<p>Оставьте свой отзыв</p>");
...
<tr><td colspan="2" class="space">&nbsp;</td></tr>'; 
		...
		if ( !isset($options['start']) ) $options['start'] = '<h2 class="responces">' . t('Отзывы') . '</h2><table class="responces">'; 
