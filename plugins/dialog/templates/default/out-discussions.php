...
if(!isset($table_title)) $table_title='';
  
$out .= '<div class="tab_discussions"><div class="tab-dis-wrap"><div class="tab-dis-title">' . $table_title . '</div>';
...
if (isset($discussions[0]['watch_comments_count']))
   $out .= '<tr><th>Дискуссия</th><th width="15%">Сообщений</th><th width="6%">Вы</th><th width="10%">Просмотров</th><th width="15%">Последнее</th></tr>';
else 
...
foreach ($discussions as $discussion)
{
  ...
  if ($comuser_id)
  {
    ...
    if ($watch_status) $watch_status = '<span class="disc_watch_status">' . $watch_status . '</span> ';
     ...
     if ($discussion_parent_comment_id)
        $status .= '<span class="disc_parent"><a href="' . $siteurl . $options['goto_slug'] . '/disc/0/comm/' . $discussion_parent_comment_id . '" title="' . $options['parent_comment'] . '">' . $options['parent'] . '#' . $discussion_parent_comment_id . '</a></span>';
     if (!$discussion_approved) $status .= '<span class="disc_need_approved">' . $options['need_approved'] . '</span>';
     if ($discussion_closed) $status .= '<span class="disc_closed">' . $options['disc_closed'] . '</span>';
     if ($discussion_private) $status .= '<span class="disc_private">' . $options['disc_private'] . '</span>';
     if ($autor) $status .= '<span class="disc_you_autor">' . $options['you_autor'] . '</span>';
     // подготовим статус к выводу
     if ($status) $status = '<span class="disc_status">' . $status . '</span>';
  ...
  */
  ...
  if (isset($watch_comments_count))  $out .= '<td class="you">' . $watch_comments_count . '</td>';
