...
// в этом файле подготавливается вывод комментов
  ...
  // формируем ссылку на создание приватной дискуссии
  // если кол-во сообщений текущего пользователя больше заданного
  
  if ( ($comuser['profile_comments_count'] > $options['comments_count_allow_private']) and ($comuser_id != $comment_creator_id) ) 
     $user_links .= '<a href="' . $siteurl . $options['main_slug'] . '/' . $options['new_private_slug'] . '?uid=' . $comment_creator_id . '" title ="' . $options['new_private'] . '"><img src="' . $template_url . 'images/private.png" alt="' . $options['new_private'] . '"></a>';  
  
  
  $user_comments_count = '<a href="' . $siteurl . $options['profile_slug'] . '/' . $comment_creator_id . '/all/' . $options['comments_slug_profile'] . '" title ="' . $options['show_comments'] . '">' . $options['user_count'] . $profile_comments_count . '</a>';
  
  // благодарности
  if ($profile_dankes) $user_dankes = '<a href="' . $siteurl . $options['profile_slug'] . '/' . $comment_creator_id . '/all/' . $options['dankes_slug_profile'] . '" title ="' . $options['show_dankes'] . '">' . $options['user_dankes'] . $profile_dankes . '</a>';
  else $user_dankes = '';
