...
if ($discussion)
{
 ...
 // $flag_show_comments_js = true; // вывести в head js плагинов комментариев 
  ...
  /* 
   ...
   {
	 $new_comm = $MSO->data['session']['comments']; // массив: id-коммент
	 
	 // получаем комментарии для этого юзера
	 $new_comments = dialog_get_comment(array('new_comments'=>$new_comm[]));
	// comment_id
	 // добавляем в вывод
	 if ($new_comments) $comments = array_merge(array($comments, $an_comments);
   }
*/
...
// вывод  _____________________________________________________________ 
  ...
  $out .= '<div class="breadcrumbs">' . $main_link  . $options['breadcrumbs_razd'] . $category_link . $discussion['discussion_title'] . '</div>';
   ...
   $out .= '<H2>' . $discussion['discussion_title']  . '</H2>';
 ...
 // выведем кнопки для управления внешним видом дискуссии         
   ...
   if ($comments)
   {
      ...
      $flag_discussion = true; // флаг что мы в дискуссии
