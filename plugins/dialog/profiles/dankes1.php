...
//$get_events_user - здеесь моет быть указан конкретный пользователь
  ...
  require_once( getinfo('plugins_dir') . 'dialog/functions/functions.php' );
    ...
    // получим все благодарности, выраженные пользователю
	  $CI = & get_instance();
	  ...
	  if ($get_events_user) $CI->db->where('gud_autor_id', $get_events_user);
	  
	  if($last_date)
	    $CI->db->where('gud_date <' , $last_date );
	  	  
	  $CI->db->join('dcomments', 'dcomments.comment_id = dgud.gud_comment_id');
	  // возможно есть фильтр по субъекту, который выразил благодарность
	  if ($subject_id) $CI->db->where('gud_user_id', $subject_id);
	...
	if ($query->num_rows() > 0)
	{
      ...
      $dialog_options['comment_creator_id'] = $comment['comment_creator_id'];
			...
			$profiles_events[$comment['gud_date']] = array(
			  ...
			  $comment['gud_autor_id'],   // 1-Пользователь (автор события)
			 ...
			 $comment['gud_user_id'],      // 5-автор элемента
