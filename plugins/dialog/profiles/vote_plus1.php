...
//$get_events_user - здеесь моет быть указан конкретный пользователь
  ...
  require_once( getinfo('plugins_dir') . 'dialog/functions/functions.php' );
    ...
    // получим все + голоса, выраженные пользователю
	  $CI = & get_instance();
	  ...
	  if ($get_events_user) $CI->db->where('vote_autor_id', $get_events_user);
	 ...
	 // $CI->db->join('ddiscussions', 'dcomments.comment_discussion_id = ddiscussions.discussion_id');
	  ...
	  // возможно есть фильтр по субъекту, который  голосовал
	  if ($subject_id) $CI->db->where('vote_user_id', $subject_id);
	...
	if ($query->num_rows() > 0)
	{
      ...
      $dialog_options['comment_creator_id'] = $comment['comment_creator_id'];
			...
			$profiles_events[$comment['vote_date']] = array(
			  ...
			  $comment['vote_autor_id'],   // 1-Пользователь (автор события)
			 ...
			 $comment['vote_user_id'],      // 5-автор элемента
