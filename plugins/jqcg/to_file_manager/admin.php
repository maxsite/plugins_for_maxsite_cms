...
function upload_block($allowed_types, $current_dir){
	...
	echo
		...
		<p>' . t('Добавьте файлы в список и нажмите кнопку "Start upload". Максимальный размер одного файла ', 'admin') . ' ' . ini_get ('post_max_size') . '.</p>
...
function files_block($allowed_types, $current_dir, $mso_descritions, $show_size){
	...
	foreach ($dirs as $file)
	{
		...
		$filesize = '';
		$imagesize = '';
		//lefro: изменено, чтобы разделить размеры изобр. ($imagesize) и вес файла ($filesize)
		if($show_size == '1'){
			if ( $ext == 'jpg' or $ext == 'jpeg' or $ext == 'gif' or $ext == 'png'  )$imagesize = getImageSizeInfo($uploads_dir . $file);
			$filesize = ' '. getFileSizeInfo($uploads_dir . $file);
		}
		//lefro конец изменения
		...
		$sel = form_checkbox('f_check_files[]', $file, false,
			...
			. '"> '
			. $file . $filesize . $imagesize . $title_f . '</label>';//lefro: $size заменён на $filesize и $imagesize
		...
		{
			if ($title) 
				$cod2 = stripslashes(htmlspecialchars( '<a title="' . $title_f . '" href="' . $uploads_url . $file . '">' . $title . '</a>') );
			else 
		...
		if ( $ext == 'jpg' or $ext == 'jpeg' or $ext == 'gif' or $ext == 'png'  )
		{
			...
			//lefro: добавлен код слайда jqcoolgallery
			if ($file)
			$cod4 = stripslashes(htmlspecialchars('[slide=' . $file . ' '  . $title . ']' . $uploads_dir . $file . ' ' . 'width=""' . ' ' . 'height=""' . ' ' . $uploads_dir . 'mini/' . $file . '[/slide]'));
			
			$cod .= ' | <a class="codeslide" href="#"
			onClick = "jAlert(\'<textarea cols=60 rows=6>' . $cod4 . '</textarea>\', \'jqcg_slide\'); return false;">Код слайда jqcoolgallery</a>';
			//lefro: конец добавления
			

			$predpr = '<a class="lightbox" href="' . $uploads_url . $file . '" target="_blank" title="' . htmlspecialchars($title) . ' ('. $file . ')' . '"><img class="file_img" style="float:left; margin-right:10px;" alt="" src="' . $uploads_url . $_f . '"></a>';
		}
		else
		{
			if (in_array($ext, $ext_w_image))
			{
				...
				$predpr = '<a href="' . $uploads_url . $file . '" target="_blank" title="' . $title . ' ('. $file . ')' . '"><br><img class="file_img" style="float:left;" alt="" src="' . $ext_imgdir_url . $ext_img . '.png"></a>';
			...
			{
				$predpr = '<a href="' . $uploads_url . $file . '" target="_blank" title="' . $title . ' ('. $file . ')' . '"><br><img class="file_img" style="float:left;" alt="" src="' . getinfo('admin_url') . 'plugins/admin_files/document_plain.png"></a>';
			}
		...
		// конец добавления
		$out_all .= '<div class="cornerz"><div class="wrap" style="max-width:550px; min-height:130px;">' . $sel . $predpr . $cod . '</div></div>';
...
//	}
	$('.edit_descr_link').toggle(function () 
	{
		if (!$(this).parent().parent().children('.edit_descr').size())
		{
			...
			var form_code = '<div class="edit_descr" style="max-width: 350px; margin-left:110px;" style="display:none"><form action="" method="post">{$session}<input type="hidden" name="f_file_name" value="' + file_name + '"><textarea name="f_file_description" >' + old_descr + ' ' + '</textarea><br><input type="submit" name="f_edit_submit" value="{$save_button}"></form></div>';
...
//lefro: добавлен код генерации пакета слайдов jqcoolGallery
	$('#jqcoolgallerycodeclick').click(function()
	{


		$('#jqcoolgallerycode').html('');
					...
					
		{
			...
			codeslide = codeslide + '[slide=' + i + ' ' + t + ']{$up}' + this.value + ' width=""' + ' height="" ' + '{$up}mini/' + this.value + '[/slide]{$n}';
			...
			if (codeslide)
			{
				...
				$('#jqcoolgallerycode').html(codeslide);
				
				
				$('#jqcoolgallerycode').css({ background: '#F0F0F0', width: '100%', height: '150px',
									border: '1px solid gray', margin: '20px 0',
									'font-family': 'Courier New',
									'font-size': '9pt'});
				

			$('#jqcoolgallerycode').fadeIn('slow');
			$('#jqcoolgallerycode').select();
			}
			else
			{
			$('#jqcoolgallerycode').hide();
			alert('{$mess}');
			}
		});
...
EOF;
		...
		echo '<h2 class="br">' . t('Создание пакета слайдов jqcool-галереи', 'admin') . '</h2>

		<p>' . '<input class="br" type="button" id="jqcoolgallerycodeclick" value="' . t('Генерировать коды слайдов jqcool-галереи', 'admin') . '">
		</p>
		<p><textarea id="jqcoolgallerycode" class="float-parent" style="display: none"></textarea>
		';
...
function getImageSizeInfo($file){
	...
	//lefro: изменено представление аттрибутов файла (label)
	return $attr = ' ' . 'width="' . $x . '" ' . 'height="' . $y . '" ';
	//lefro: конец изменения
}
