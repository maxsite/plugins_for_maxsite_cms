...
function category_ext_autoload($args = array())
{
	# регистрируем виджет
	mso_register_widget('category_ext_widget', t('Рубрики')); 
}

# функция выполняется при деинсталяции плагина
function category_ext_uninstall($args = array())
{	
	mso_delete_option_mask('category_ext_widget_', 'plugins' ); // удалим созданные опции
	return $args;
}


# функция, которая берет настройки из опций виджетов
function category_ext_widget($num = 1) 
{
	$widget = 'category_ext_widget_' . $num; // имя для опций = виджет + номер
	...
	return category_ext_widget_custom($options, $num);
}


# форма настройки виджета 
# имя функции = виджет_form
function category_ext_widget_form($num = 1) 
{

	$widget = 'category_ext_widget_' . $num; // имя для формы и опций = виджет + номер
	...
	if ( !isset($options['date_now']) ) $options['date_now'] = 0;
	...
	$form .= mso_widget_create_form(t('Включать потомков'), form_dropdown( $widget . 'include_child', 
				...
				'2'=>t('Исключить родительскую рубрику и включить потомков только первого уровня'),
				'-1'=>t('Исключить всех')
				), $options['include_child']), '');

	$form .= mso_widget_create_form(t('Ориентироваться на дату публикации постов в рубрике'), form_dropdown( $widget . 'date_now', 
				array( 
				'0'=>t('Нет'), 
				'1'=>t('Да'),
				), $options['date_now']), '');
...
function category_ext_widget_update($num = 1) 
{
	...
	$newoptions['date_now'] = mso_widget_get_post($widget . 'date_now');
...
function category_ext_widget_custom($options = array(), $num = 1)
{
	...
	if( isset($first_level) && $first_level )
	{
		$new_all = array();
		foreach( $all as $parent_cat )
		{
			if( isset($parent_cat['childs']) && is_array($parent_cat['childs']) )
			{
				foreach( $parent_cat['childs'] as $child )
				{
					if( isset($child['childs']) && is_array($child['childs']) )
					{
						unset($child['childs']);
					}
					$new_all[$child['category_id']] = $child;
				}
			}
		}
		#pr($new_all);
