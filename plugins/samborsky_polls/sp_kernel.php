<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	class sp_question{
		...
		function results(){
			...
			$CI->table->add_row("<b>".t('Всего проголосовало:')."</b> {$total} ".t('чел'));

			if( $this->options['show_archives_link'] )
				$CI->table->add_row('<div align="left"><a href="'.getinfo('siteurl').$this->options['archive_url'].'">'.t('Архивы голосований').'</a></div>');
		...
		function form(){
			...
			$CI->table->add_row(
				...
				. '<input class="btn_poll" type="button" value="'.t('Проголосовать').'" onclick="javascript:sp_polls_vote('.$this->id.');"><div class="break"></div>' . $results_link
			);
