...
$default = array(
        ...
        'head_title' => t('meta-title опросов'),
		'head_description' => t('meta-description опросов'),
		'head_keywords' => t('meta-keywords опросов'),
		'name_header' => t('h1 опросов')
...
if(!$query->num_rows())
{
	...
	$CI->db->or_where('options_key', 'head_title');
	$CI->db->or_where('options_key', 'head_description');
	$CI->db->or_where('options_key', 'head_keywords');
	$CI->db->or_where('options_key', 'name_header');
	...
	foreach($query->result() as $row)
	{
		if($row->options_type == 'plugins')
		{
			switch ($row->options_key)
			{
				...
				case 'head_title' : $default[''] = $row->options_value; break;
				case 'head_description' : $default[''] = $row->options_value; break;
				case 'head_keywords' : $default[''] = $row->options_value; break;
				case 'name_header' : $default[''] = $row->options_value; break;
...
mso_admin_plugin_options('plugin_samborsky_polls', 'plugins',
	array(
        ...
        # SEO параметры страницы с формой ввода
			'group_1' => array(
				'type' => 'info',
				'title' => 'SEO параметры страницы архива голосований',
				'text' => '',
			),
			...
			'head_title' =>  Array(
				'type' => 'text',
				'name' => 'TITLE',
				'description' => 'Содержимое тэга TITLE для страницы <b>Архива Голосований</b>. Если поле оставить пустым, то тэг <b>&lt;title&gt;</b> останется незаполненным.',
				'default' =>  'Архив Голосований.'
			),
			'head_description' =>  Array(
				'type' => 'text',
				'name' => 'DESCRIPTION',
				'description' => 'Значение мета-тэга DESCRIPTION.',
				'default' => 'Архив всех Голосований.'
			),
			'head_keywords' =>  Array(
				'type' => 'text',
				'name' => 'KEYWORDS',
				'description' => 'Значение мета-тэга KEYWORDS.',
				'default' => 'архив,Голосований,опрос,опросы,мнения,'
			),
			'name_header' =>  Array(
				'type' => 'text',
				'name' => 'h1 Голосований',
				'description' => 'h1 на странице Голосований.',
				'default' => 'Архив Голосований'
			),
