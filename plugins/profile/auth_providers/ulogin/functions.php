...
{
  if (strstr($email, 'aaa.fff')) return true;
  else return false;
}
...
function ulogin_comuser_auth($data)
{
  // разберемся с провайдерами
   ...
   if ($provider_key and !$email) // на всякий случай сконструируем email
     $email = 'no';
 ...
 {  
   // если указанный email зарегистрирован на user, то удаляем его
   $CI->db->select('users_id');
   $CI->db->where( 'users_email', $email);
   $query = $CI->db->get('users');
   if ($query->num_rows() > 0) # есть
   {
	  $email = ulogin_create_email(array('provider_key'=>$provider_key, 'provider_user_key'=>$provider_user_key)); 
   ...
   elseif ($allow_create_new_comuser) // только если разрешено создавать новых комюзеров
   {
     ...
     if ($query->num_rows()) // есть такой комюзер
     {
        ...
        die(t('Email, переданный социальной сетью, уже зарегистрирован в системе! ', 'plugins') . $email . $provider_key . ' ' . $provider_user_id);
      }      
   ...
   else die(t('Запрещено создавать новых комюзеров'));
...
function ulogin_providers_visible($comusers_id = 0, $prowiders_visible = '')
{
	if (!$comusers_id) return true; // есть ошибки
	
	$prowiders_visible = str_replace(",," , "," , $prowiders_visible);
	$prowiders_visible = explode(',' , $prowiders_visible);
	$prowiders_visible = implode(',' , $prowiders_visible);
   $CI = & get_instance();

   // вначале грохаем если есть такой ключ
   $CI->db->where('meta_table', 'comusers');
   $CI->db->where('meta_id_obj', $comusers_id);
   $CI->db->where('meta_key', KEY_PROVIDERS_VISIBLE);
   $CI->db->delete('meta');
         
  // теперь добавляем как новый
  // если есть отображаемые
  if ($prowiders_visible) 
  {
       $ins_data2 = array(
               'meta_table' => 'comusers',
               'meta_id_obj' => $comusers_id,
               'meta_key' => KEY_PROVIDERS_VISIBLE,
               'meta_value' => $prowiders_visible
               );
