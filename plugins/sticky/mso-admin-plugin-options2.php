...
# 
if( !function_exists('mso_admin_plugin_options2') )
{
	function mso_admin_plugin_options2($key, $type, $ar, $title = '', $info = '', $text_other = '', $show_goto_plugins = false)
	{
		...
		if( $ar ) # есть опции
		{
			...
			foreach( $ar as $m => $val )
			{
				...
				if( $val['description'] ) $val['description'] = '<p class="nop '.$m.'"><span class="fhint">'.$val['description'].'</span></p>';
