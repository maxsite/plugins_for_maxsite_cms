<?php if (!defined('BASEPATH')) exit('No direct script access allowed'); 
		...
		
	{
		...
		if ($CI->db->update('other_comments', array('comments_approved'=>$action) ) )
		{
			mso_flush_cache();
...
<p><strong><?= t('Фильтр:') ?></strong> <a href="<?= getinfo('site_admin_url') ?>other_comments/all"><?= t('Все') ?></a> | <a href="<?= getinfo('site_admin_url') ?>other_comments/moderation"><?= t('Только требующие модерации') ?></a></p>
...
<?php
	...
	$CI->table->set_heading('ID', '+', t('Комментарий'),  t('Действие'));
	
	# подготавливаем выборку из базы
	
	$CI->db->select('SQL_CALC_FOUND_ROWS comments_id, comments_users_id, comments_comusers_id, comments_author_name, comments_date, comments_content, comments_approved, comments_author_ip, users.users_nik, comusers.comusers_nik, elements.element_title, elements.element_slug , kinds.kind_slug , kinds.kind_title', false);
	...
	if ($query->num_rows() > 0)
	{
		$this_url = getinfo('site_admin_url') . 'other_comments/';
		...
		foreach ($query->result_array() as $row)
		{
			...
			$id_out = $id . ' <input type="checkbox" name="f_check_comments[' . $id . ']">' . NR;
			
			$act = '<a href="' . $this_url . 'comment-edit/'. $id . '">' . t('Изменить') . '</a>';
			...
			$page_slug = $row['kind_slug'] . '/' . $row['element_slug'];
			$page_title = '<a target="_blank" href="' . $site_url . $page_slug . '#comment-' . $id . '">' . htmlspecialchars( $row['element_title'] ) . '</a>';
			...
			$CI->table->add_row($id_out, $comments_approved, $out, $act);
		}
	}
