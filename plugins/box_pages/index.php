...
function box_pages_autoload($args = array())
{
	# регистрируем виджет
	mso_register_widget('box_pages_widget', t('Блок статических страниц'));
}

# функция выполняется при деинсталяции плагина
function box_pages_uninstall($args = array())
{
	mso_delete_option_mask('box_pages_widget_', 'plugins' ); // удалим созданные опции
	return $args;
}

# функция, которая берет настройки из опций виджетов
function box_pages_widget($num = 1)
{
	$widget = 'box_pages_widget_' . $num; // имя для опций = виджет + номер
	...
	return box_pages_widget_custom($options, $num);
}


# форма настройки виджета
# имя функции = виджет_form
function box_pages_widget_form($num = 1)
{

	$widget = 'box_pages_widget_' . $num; // имя для формы и опций = виджет + номер
	...
	if ( !isset($options['page_type']) )		$options['page_type'] = 'static';
...
function box_pages_widget_update($num = 1)
{
...
function box_pages_widget_custom($arg = array(), $num = 1)
{
	if (!isset($arg['count'])) 			$arg['count'] = 7;
	if (!isset($arg['page_type']))  	$arg['page_type'] = 'static';
	...
	if ( !isset($arg['header']) ) $arg['header'] = mso_get_val('widget_header_start', '<h2 class="box"><span>') . t('Блок статических страниц') . mso_get_val('widget_header_end', '</span></h2>');

	if ( !isset($arg['block_start']) ) $arg['block_start'] = '<div class="box-pages"><ul class="is_link">';
	if ( !isset($arg['block_end']) ) $arg['block_end'] = '</ul></div>';



	$cache_key = 'box_pages_widget'. serialize($arg) . $num;
