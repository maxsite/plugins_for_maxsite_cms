...
function super_pages_autoload($args = array())
{
	# регистрируем виджет
	mso_register_widget('super_pages_widget', t('Супер Страницы'));
}

# функция выполняется при деинсталяции плагина
function super_pages_uninstall($args = array())
{
	mso_delete_option_mask('super_pages_widget_', 'plugins' ); // удалим созданные опции
	return $args;
}

# функция, которая берет настройки из опций виджетов
function super_pages_widget($num = 1)
{
	$widget = 'super_pages_widget_' . $num; // имя для опций = виджет + номер
	...
	return super_pages_widget_custom($options, $num);
}


# форма настройки виджета
# имя функции = виджет_form
function super_pages_widget_form($num = 1)
{
	$widget = 'super_pages_widget_' . $num; // имя для формы и опций = виджет + номер
	...
	if ( !isset($options['img_prev']) )			$options['img_prev'] = 'prev';
	if ( !isset($options['img_prev_def']) )		$options['img_prev_def'] = '';
	if ( !isset($options['img_prev_attr']) )	$options['img_prev_attr'] = 'class="center"';
	if ( !isset($options['img_prev_width']) )	$options['img_prev_width'] = '100';
	if ( !isset($options['img_prev_height']) )	$options['img_prev_height'] = '100';
	if ( !isset($options['max_words']) )		$options['max_words'] = 20;
	if ( !isset($options['text_posle']) ) 		$options['text_posle'] = '';
	if ( !isset($options['days']) )				$options['days'] = '999999';
	...
	$form .= mso_widget_create_form(t('Формат'), form_input( array( 'name'=>$widget . 'format', 'value'=>$options['format'] ) ), '%TITLE% %DATE% %TEXT% %TEXT_CUT% %TEXT_PREV% %IMG_PREV% %COMMENTS% %URL% %VIEW% %RATING%');
	...
	$form .= mso_widget_create_form(t('Сортировка'), form_dropdown( $widget . 'sort', array( 'page_date_publish'=>t('По дате'), 'page_title'=>t('По алфавиту'), 'page_view_count'=>t('По просмотрам'), 'page_count_comments'=>t('По комментариям'), 'page_sredn'=>t('По рейтингу')), $options['sort']), '');
	...
	$form .= mso_widget_create_form(t('Ширина миниатюры'), form_input( array( 'name'=>$widget . 'img_prev_width', 'value'=>$options['img_prev_width'] ) ), t('ширина генерируемого превью в пикселях'));

	$form .= mso_widget_create_form(t('Высота миниатюры'), form_input( array( 'name'=>$widget . 'img_prev_height', 'value'=>$options['img_prev_height'] ) ), t('высота генерируемого превью в пикселях'));
	...
	$form .= mso_widget_create_form(t('N дней'), form_input( array( 'name'=>$widget . 'days', 'value'=>$options['days'] ) ), t('Учитывать страницы, опубликованные за последние N дней'));
...
function super_pages_widget_update($num = 1)
{
	...
	$newoptions['img_prev_width'] = mso_widget_get_post($widget . 'img_prev_width');
	$newoptions['img_prev_height'] = mso_widget_get_post($widget . 'img_prev_height');
...
function super_pages_widget_custom($arg = array(), $num = 1)
{
	...
	if (!isset($arg['img_prev_attr'])) 	$arg['img_prev_attr'] = 'class="center"';
	if (!isset($arg['img_prev_width']))	$arg['img_prev_width'] = '100';
	if (!isset($arg['img_prev_height']))$arg['img_prev_height'] = '100';
	if (!isset($arg['max_words']) ) 	$arg['max_words'] = 20;
	if (!isset($arg['text_posle']) ) 	$arg['text_posle'] = '';
	if ( !isset($arg['header']) )		$arg['header'] = mso_get_val('widget_header_start', '<h2 class="box"><span>') . t('Страницы с генерацией превью') . mso_get_val('widget_header_end', '</span></h2>');
	if ( !isset($arg['block_start']) )	$arg['block_start'] = '<div class="last-pages">';
	if ( !isset($arg['block_end']) )	$arg['block_end'] = '</div>';
	if ( !isset($arg['days']) )			$arg['days'] = '999999';

	$cache_key = 'super_pages_widget'. serialize($arg) . $num;
	...
	$CI = & get_instance();
	// текст будет прогонятся через стандартный парсер
	...
	if (strpos($arg['format'], '%TEXT%') === false	&& strpos($arg['format'], '%TEXT_CUT%') === false && strpos($arg['format'], '%TEXT_PREV%') === false)
		$CI->db->select('page.page_id, page_type_name, page_type_name AS page_content, page_slug, page_title, page_view_count, page_date_publish, page_status, round(page_rating/page_rating_count) AS page_sredn, COUNT(comments_id) AS page_count_comments', false);
	else
		$CI->db->select('page.page_id, page.page_content, page_type_name, page_slug, page_title, page_view_count, page_date_publish, page_status, round(page_rating/page_rating_count) AS page_sredn, COUNT(comments_id) AS page_count_comments');
	...
	$CI->db->where('page_date_publish >', date('Y-m-d H:i:s', time()-$arg['days']*24*60*60));
		...
		foreach ($pages as $key => $page)
		{
			...
			if (strpos($arg['format'], '%IMG_PREV%') !== false)
			{
				if ($img_prev = mso_get_meta($arg['img_prev'], 'page', $page['page_id']))
				{
					...
					{
						if ( $img = thumb_generate(
								$img_prev[0]['meta_value'], // адрес
								$arg['img_prev_width'], //ширина
								$arg['img_prev_height'], //высота
								'', // если нет
								'resize_full_crop_center', 
								mso_get_val('refresh_thumb') )
							)
								$img_prev = '<img src="' . $img . '" alt="" ' . $arg['img_prev_attr'] . '> ';
					}
					else
			...
			$out = str_replace('%VIEW%', $page['page_view_count'], $out);

			$out = str_replace('%RATING%', $page['page_sredn'], $out);
			...
			$page_content = mso_hook('content_in', $page_content);
			$page_content = mso_hook('content', $page_content);
			$page_content = mso_hook('content_out', $page_content);
			
			// обработка парсером
			$parser_content = mso_get_meta('parser_content', 'page', $page['page_id']);
			if ( isset($parser_content[0]['meta_value']) )
			{
				if ($parser_content[0]['meta_value'] !== 'none')
				{
					$p = $parser_content[0]['meta_value'];
					
					if (isset($parser_all[$p]['content']))
					{
						$func = $parser_all[$p]['content']; // функция, которую нужно выполнить
						if ( function_exists($func) ) $page_content = $func($page_content);
					}
				}
			}
			else
			{
				$page_content = parser_default_content($page_content);
			}
		
			$page_content = mso_hook('content_complete', $page_content);
				
			$out = str_replace('%TEXT%', $page_content, $out);
			...
			$out = str_replace('%TEXT_CUT%', $page_content, $out);
