...
function editor_dumb_mod_autoload($args = array())
{
	mso_hook_add( 'editor_custom', 'editor_dumb_mod'); # хук на подключение своего редактора
}

# функция выполняется при деинсталяции плагина
function editor_dumb_mod_uninstall($args = array())
{	
	mso_delete_option('editor_dumb_mod', 'plugins'); // удалим созданные опции
	return $args;
}

function editor_dumb_mod($args = array()) 
{
	
	$options = mso_get_option('editor_dumb_mod', 'plugins', array() ); // получаем опции
	
	$editor_config['url'] = getinfo('plugins_url') . 'editor_dumb_mod/';
	$editor_config['dir'] = getinfo('plugins_dir') . 'editor_dumb_mod/';
...
function editor_dumb_mod_mso_options() 
{
	mso_admin_plugin_options('editor_dumb_mod', 'plugins', 
		array(
			'editor' => array(
							'type' => 'select', 
							'name' => 'Редактор', 
