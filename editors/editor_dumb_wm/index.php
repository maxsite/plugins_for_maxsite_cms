...
function editor_dumb_wm_autoload($args = array())
{
	mso_hook_add( 'editor_custom', 'editor_dumb_wm'); # хук на подключение своего редактора

}

# функция выполняется при деинсталяции плагина
function editor_dumb_wm_uninstall($args = array())
{
	mso_delete_option('editor_dumb_wm', 'plugins' ); // удалим созданные опции
	return $args;
}

function editor_dumb_wm($args = array())
{

	$options = mso_get_option('editor_dumb_wm', 'plugins', array() ); // получаем опции

	$editor_config['url'] = getinfo('plugins_url') . 'editor_dumb_wm/';
	$editor_config['dir'] = getinfo('plugins_dir') . 'editor_dumb_wm/';
...
function editor_dumb_wm_mso_options()
{
	mso_admin_plugin_options('editor_dumb_wm', 'plugins',
		array(
			'editor' => array(
							'type' => 'select',
							'name' => t('Редактор'),
