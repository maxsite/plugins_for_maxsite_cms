...
else
{
	...
	$thumbs_url			= $thumbs_base_url.$rfm_subfolder;
...
$class_ext = '';
$src = '';
//pr($current_path.$rfm_subfolder.$subdir);
	...
	<div class="span12">
		...
		foreach ($files as $file_array) {
			...
			$file=$file_array['file'];
				...
				$mini_src = $src_thumb = $thumbs_url.$subdir. $file;
				$mini_src_dir = $src_thumb_dir = $thumbs_path.$subdir. $file;
				//add in thumbs folder if not exist
				if(!file_exists($src_thumb_dir)){
				try {
					if(!create_img($file_path, $src_thumb_dir, 122, 91)){
							$src_thumb=$mini_src="";
					}else{
